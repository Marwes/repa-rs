...
pub fn map<S, F, B>(f: F, array: S) -> DArray<<S as Source>::Shape, MapFn<S, F>>
    where F: Fn(<S as Source>::Element) -> B,
          S: Source
{
    from_select(array.extent().clone(),
                MapFn {
                    source: array,
                    f: f,
                })
...
pub fn extract<S>(start: <S as Source>::Shape,
                  size: <S as Source>::Shape,
                  array: S)
                  -> DArray<<S as Source>::Shape, ExtractFn<S>>
    where S: Source
{
...
pub fn zip_with<S1, S2, F, O>(lhs: S1,
                              rhs: S2,
                              f: F)
                              -> DArray<<S1 as Source>::Shape, ZipWithFn<S1, S2, F>>
    where S1: Source,
          S2: Source<Shape = <S1 as Source>::Shape>,
          F: Fn(<S1 as Source>::Element, <S2 as Source>::Element) -> O
{
    from_select(lhs.extent().intersect_dim(rhs.extent()),
                ZipWithFn {
                    lhs: lhs,
                    rhs: rhs,
                    f: f,
...
pub fn traverse<S, Sh, F, T, A, B>(array: S,
                                   new_shape: F,
                                   transform: T)
                                   -> DArray<Sh, TraverseFn<S, T>>
    where S: Source,
          Sh: Shape,
...
pub fn slice<A, S>(array: A, slice: S) -> DArray<<S as Slice>::Slice, SliceFn<A, S>>
    where A: Source<Shape = <S as Slice>::Full>,
          S: Slice
{
    from_select(slice.slice_of_full(array.extent().clone()),
                SliceFn {
                    array: array,
                    slice: slice,
                })
...
pub fn fold_s<F, S, Sh>(mut f: F,
                        e: <S as Source>::Element,
                        array: &S)
                        -> UArray<Sh, Vec<<S as Source>::Element>>
    where Sh: Shape,
...
pub fn fold_p<F, S, Sh>(ref f: F,
                        ref e: <S as Source>::Element,
                        array: &S)
                        -> UArray<Sh, Vec<<S as Source>::Element>>
    where Sh: Shape,
          S: Source<Shape = Cons<Sh>>,
...
mod tests {
              ...
              B: Source<Shape = Cons<Cons<Z>>, Element = A::Element>
    {
        ...
        fn select(&self, (sh,): (&<A as Source>::Shape,)) -> B::Element {
            ...
            iter(&zip_with(&row, &column, |l, r| l * r)).fold(0, |l, r| l + r)
        }
    }
